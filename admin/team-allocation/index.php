...
$sm = $month - 1;
if ($month == 12) {
    $urlma = $url . "/admin/team-allocation/index.php?m=1&y=" . $ay;
    $amonth = 1;
} else {
    $urlma = $url . "/admin/team-allocation/index.php?m=" . $am . "&y=" . $year;
    $amonth = $am;
}

if ($month == 1) {
    $urlms = $url . "/admin/team-allocation/index.php?m=12&y=" . $sy;
    $smonth = 12;
} else {
    $urlms = $url . "/admin/team-allocation/index.php?m=" . $sm . "&y=" . $year;
    $smonth = $sm;
}
...
<div class="card card-bg mb-1">
    <div class="card-body pt-3 pb-2 d-flex justify-content-between align-items-center">
        ...
        <h5>
            <div class="input-group">
                <a href="<?php echo $url . "/admin/team-allocation/index.php?m=" . $month . "&y=" . $sy ?>" class="btn btn-sm btn-outline-warning"><i class="fas fa-minus"></i></a>
                ...
                <a href="<?php echo $url . "/admin/team-allocation/index.php?m=" . $month . "&y=" . $ay ?>" class="btn btn-sm btn-outline-warning"><i class="fas fa-plus"></i></a>
            </div>
        </h5>
    </div>
</div>
...
<div class="table-responsive card-bg rounded shadow mt-0">
    <table class="table table-sm table-bordered">
        <thead>
            ...
            <th class='text-warning text-center'>Location</th>
        ...
        <tbody>
            ...
            <?php
            $sqltr = "SELECT * FROM events_db LEFT JOIN clients_db ON events_db.event_cl_id=clients_db.c_id WHERE events_db.cus_id={$cusID} AND month(event_date)='$month' AND year(event_date)='$year' ORDER BY events_db.event_date ASC";
