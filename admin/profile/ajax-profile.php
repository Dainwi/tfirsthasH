...
if (isset($_POST['updatepro'])) {
    ...
    $sql = "UPDATE admin SET c_name ='$name', c_phone= '$phone', c_email ='$email', address='$address' WHERE c_id={$userid}";
    ...
    if ($error === 0) {
        ...
        if ($img_size > 10485760) {
            ...
            if (in_array($img_ex_lc, $allowed_exs)) {
                ...
                if ($move_file) {
                    $sql1 = "UPDATE admin SET c_photo='$new_img_name' WHERE c_id={$uid}";
...
if (isset($_POST['new-password'])) {
    ...
    if ($npass === $cpass) {
        ...
        $sql = "SELECT password,c_id FROM admin WHERE c_id ={$aid} AND password='$oldpass'";
        $res = mysqli_query($con, $sql);
        if (mysqli_num_rows($res) > 0) {

            $sql2 = "UPDATE admin SET password='$newpass' WHERE c_id={$aid}";
