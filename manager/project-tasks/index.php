...
$sql1 = "SELECT * FROM deliverables_assign LEFT JOIN deliverables_db ON deliverables_db.d_id=deliverables_assign.da_d_id LEFT JOIN assign_manager_db ON deliverables_assign.da_pr_id=assign_manager_db.am_project_id LEFT JOIN projects_db ON deliverables_assign.da_pr_id = projects_db.project_id  WHERE da_status=0 AND project_status =1 AND assign_manager_db.am_user_id={$manager_id} ORDER BY da_due_date ASC LIMIT {$offset},{$limit}";
...
if (mysqli_num_rows($res1) > 0) {
    ...
    <th class='text-warning'>Client</th>
        ...
        <th class='text-warning'>View Task</th>
    ...
    while ($row1 = mysqli_fetch_assoc($res1)) {
        ...
        $prid = $row1['da_pr_id'];
        $sqlcl = "SELECT * FROM projects_db LEFT JOIN clients_db ON clients_db.c_id=projects_db.project_client WHERE project_id={$prid}";
        ...
        $response .= "<tr>";
        $response .= "<td class='text-center'><a href='{$url}/manager/projects/view-project.php?prid={$prid}'>{$clientname}</a></td>";
...
<div class="card-footer">
    <nav aria-label="Page navigation example">
        ...
        $sql_pg = "SELECT * FROM deliverables_assign LEFT JOIN assign_manager_db ON deliverables_assign.da_pr_id=assign_manager_db.am_project_id WHERE da_status=0 AND assign_manager_db.am_user_id={$manager_id} ";
        ...
        if (mysqli_num_rows($res_pg) > 0) {
            ...
            if ($page > 1) {
                echo "<li class='page-item'><a class='page-link' href='{$url}/manager/project-tasks/index.php?pg=" . ($page - 1) . "'>Prev</a></li>";
            } else {
            ...
            for ($i = 1; $i <= $total_page; $i++) {
                ...
                echo '<li class="page-item ' . $active . '"><a class="page-link" href="' . $url . '/manager/project-tasks/index.php?pg=' . $i . '">' . $i . '</a></li>';
            }
            if ($total_page > $page) {
                echo "<li class='page-item'><a class='page-link' href='{$url}/manager/project-tasks/index.php?pg=" . ($page + 1) . "'>Next</a></li>";
            } else {
...
$(".btn-del-da").click(function(e) {
    ...
    var urll = "<?php echo $url . '/manager/projects/view-project-ajax.php ' ?>";
    $.ajax({
        type: "post",
        url: urll,
        data: {
